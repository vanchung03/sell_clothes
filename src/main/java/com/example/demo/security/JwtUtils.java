...
public class JwtUtils {
    ...
    private final long accessTokenExpirationMs = 1000 * 60 * 3;  // 3 phút
    private final long refreshTokenExpirationMs = 86400000; // 1 ngày

    public String generateAccessToken(String username, String email, List<String> roles, String status) {
        return Jwts.builder()
                .setSubject(username)
                .claim("email", email)
                .claim("roles", roles)
    ...
    public boolean validateToken(String token) {
        try {
            ...
            // Token đã hết hạn nhưng không ghi log lỗi
            System.out.println("Token đã hết hạn");
            return false;
        } catch (io.jsonwebtoken.SignatureException e) {
            // Token bị sửa đổi, chữ ký không hợp lệ
            System.out.println("Lỗi chữ ký token: " + e.getMessage());
            return false;
            ...
            System.out.println("Lỗi token khác: " + e.getMessage());
            return false;
        }
    }
